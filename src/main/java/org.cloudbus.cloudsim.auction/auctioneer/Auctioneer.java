...
public class Auctioneer extends SimEntity{
    ...
    private static Auctioneer auctioneerId ;
    //private static Auctioneer auctioneerCloudletId ;
    int i=0;
    float val=(float) 0.3;//good 0.3 
	...
	LinkedList<Cloudlet> list = new LinkedList<Cloudlet>();
	    ...
	    public List<Cloudlet> getCloudletList() {
	        return (List<Cloudlet>) list;
	    }
	
	//public static int getUserId(Vm vm) {
		//return vm.getUserId();
		//return auctioneerId.getUserId();
//	}
	 ...
	 */	
		...
		for (BidCalculation brokerBidCalc : brokerBidCalcs) {
			...
			if (brokerVMs != null) { // If broker has request
					...
					if (brokerBidCalc.getAverageBid() >= datacenterBidCalc.getAverageBid()) {
						...
						if (datacenterVMs != null) { // If datacenter has offer
							...
							this.algorithmEngine(brokerBidCalc, brokerVMs, datacenterBidCalc, datacenterVMs);						
                                                //Log.printLine(brokerBidCalc.getAverageBid());
                ...
                for (BidCalculation brokerBidCalc : brokerBidCalcs) {
                    ////////start////////
                    String string = brokerBidCalc.toString();
                                    String[] parts = string.split(":");                                    
                                    String part2 = parts[1]; // 
                                    i=i+1;
                                    float f = Float.parseFloat(part2);
			...
			if (brokerVMs != null) { // If broker has request
				for (BidCalculation datacenterBidCalc : datacenterBidCalcs) {
					 ...
					 * bid to let the broker bid accepted.
					 */      
					if (d >= datacenterBidCalc.getAverageBid()) {
						List<Vm> datacenterVMs = datacenterBidCalc.getOrderedResources();
						if (datacenterVMs != null) { // If datacenter has offer
...
*/	
//override----------algorithmEngine----------------------------------------------------  ------
     private void algorithmEngine(BidCalculation brokerBidCalc, List<Vm> requestedVMs, BidCalculation datacenterBidCalc, List<Vm> offeredVMs) {
		...
		for (Vm requestedVM: requestedVMs) {
			for (Vm offeredVM: offeredVMs) {
				...
				/*** See if offered VM can satisfy the request */
				if (VmUtils.canServe(offeredVM, requestedVM) ) { 
				     if( CalculateCpuUtilization(offeredVM , offeredVMs)) {
				    	       double uOA = compueUOA ( brokerBidCalc, requestedVM, datacenterBidCalc, offeredVM);
					 ...
					 /*** Assign mapped-together VMs */
					 this.assignVMs(brokerBidCalc, requestedVM, datacenterBidCalc, offer, assignAmount);
				        break;
				  }//if (VmUtils.canServe 	
			}// end of for (Vm offeredVM : offeredVMs)
		}// end of for (Vm requestedVM : requestedVMs)
...
//......................compute CalculateCpuUtilization..........................................................
	
	public boolean  CalculateCpuUtilization(Vm vmList , List<Vm> vmLists) {
        double[] waiting_time = new double[list.size()];
        double[] rt2 = new double[list.size()];
        double[] run_Time = new double[list.size()];
       ...
       /** ArrayList<double[]> temp = new ArrayList<double[]>();  // list of machines 
        ...
        for (int i = 0; i < list.size(); i++) {
           ...
           // for (int j = 0; j < vmLists.size(); j++) {
                Cloudlet cloudlet = list.get(i);
                rt2[i] = (double) cloudlet.getCloudletLength() 
        ...
        for (int i = 0; i < list.size(); i++) {
            double minvalue = 500;
            double min_completiontime = 0.0;

            for (int j = 0; j < vmLists.size(); j++) {
               min_completiontime =  rt.get(j)[i];
                if (min_completiontime < minvalue) {
                    minvalue = min_completiontime;
                    if (vmList.getUserId() == vmLists.get(j).getUserId()) {
                    	CandidVm = vmList;
                     }
...
//..........................................compueUOA....................................................
	...
	public double compueUOA (BidCalculation brokerBidCalc, Vm requestedVM, BidCalculation datacenterBidCalc, Vm offeredVM) {
		
		return ((brokerBidCalc.getAverageBid() - datacenterBidCalc.getAverageBid()) * 
				                                  (requestedVM.getMips() / offeredVM.getMips()));
...
//...............................................................................
  ...
  public double calculatewaitingtimeofvm(int id ) {
        double waitingtime = 0.0;
       waitingtime = getCloudletList().get(id).getWaitingTime();
       //waitingtime = getCloudletList().
        return waitingtime;
    }
