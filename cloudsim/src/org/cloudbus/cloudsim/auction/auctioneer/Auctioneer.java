...
public class Auctioneer extends SimEntity{
	...
	private void decideStartAuction() {
		 ...
		 */
		 if (!this.auctionStartNotificationSent && this.registeredEntityList.size() >= Auctioneer.MIN_REGISTERED_ENTITIES) {
	...
	private void decideCloseAuction() {
		...
		if (this.auctionStartNotificationSent && this.sentStartAuctionNotifications <= this.registeredEntityList.size()) {
	...
	private static double computePrice(BidCalculation brokerBidCalc, BidCalculation datacenterBidCalc, int assignAmount) {
	...
	private void printAllocations() {
        ...
        //this function work for both broker and data center 
    private void printBidCalcs(String name, List<BidCalculation> brokerBidCalcs) {
        System.out.println("Printing the bid calculations for " + name);//name=broker
